...
def main(
    ...
    if st.button("Нажми кнопку! Если что то уже хрустит в спине 😷",  key=3):     
        st.balloons()
        st.info("Желаем иммунитета тебе настолько сильного, что даже вирусы будут начинать тренироваться перед тем, как попытаться атаковать! А здоровье будет таким прочным, что вирусам придется делать мемы о своих неудачных попытках заразить тебя!")
